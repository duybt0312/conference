...
function RTCPeerConnection(options) {
	console.log("3:RTCPeerConnection"+options);
    var w = window,
    ...
    function createOffer() {
        ...
        peer.createOffer(function(sessionDescription) {
			console.log("3:peer.createOffer"+sessionDescription);
    ...
    function createAnswer() {
		console.log("3:createAnswer");
        if (!options.onAnswerSDP) return;

        peer.setRemoteDescription(new SessionDescription(options.offerSDP));
		console.log("3:createAnswer" + SessionDescription);
        peer.createAnswer(function(sessionDescription) {
			console.log("3:sessionDescription");
            sessionDescription.sdp = serializeSdp(sessionDescription.sdp);
            peer.setLocalDescription(sessionDescription);
    ...
    function openOffererChannel() {
		console.log("3:openOffererChannel");
        ...
        if (moz) {
            navigator.mozGetUserMedia({
				...
				console.log('3 openOffererChannel stream' + stream);
    ...
    function _openOffererChannel() {
		console.log("3:_openOffererChannel");
    ...
    function setChannelEvents() {
		console.log("3:setChannelEvents");
        channel.onmessage = function(event) {
			console.log("3:setChannelEvents onmessage" + event); 
            if (options.onChannelMessage) options.onChannelMessage(event);
        };

        channel.onopen = function() {
			console.log("3:setChannelEvents onopen" + event); 
    ...
    function openAnswererChannel() {
		console.log("3:openAnswererChannel");
        ...
        if (moz) {
            navigator.mozGetUserMedia({
				...
				console.log("3:openAnswererChannel stream" + stream);
                    peer.addStream(stream);
                    createAnswer();
                }, useless);
        }
    ...
    return {
		...
		//	console.log("3:addAnswerSDP"+ JSON.stringify(sdp));
            peer.setRemoteDescription(new SessionDescription(sdp.sdp));
        },
        addICE: function(candidate) {
			console.log("3:addICE"+candidate);
        ...
        sendData: function(message) {
			console.log("3:sendData"+message);
...
function getUserMedia(options) {
	console.log("3:getUserMedia"+options);
    var n = navigator,
        media;
    ...
    function streaming(stream) {
		console.log("3:streaming"+stream);
